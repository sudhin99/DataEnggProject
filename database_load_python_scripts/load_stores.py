...
def load_stores():
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            # Drop audit columns if present (follow same pattern as load_orders)
            for col in ['LOADED_AT', 'IS_PROCESSED', 'SOURCE_FILE', 'ROW_HASH']:
                if col in df.columns:
                    df = df.drop(columns=[col])

            # Add source file and normalize nulls so DB defaults apply for LOADED_AT
            df['SOURCE_FILE'] = file_name
            df = df.where(pd.notnull(df), None)

            rows = len(df)

            # Load Data (all-or-nothing transaction)
