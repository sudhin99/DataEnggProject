...
DROP PROCEDURE IF EXISTS sp_load_dim_store$$

CREATE PROCEDURE sp_load_dim_store()
BEGIN
    DECLARE v_started_at TIMESTAMP;
    DECLARE v_sp_name VARCHAR(100) DEFAULT 'sp_load_dim_store';
    ...
    SELECT COALESCE(MAX(updated_at), '1970-01-01') INTO v_watermark FROM RELIANT_DWH_GOLD.DIM_STORE;

    SELECT MAX(updated_at) INTO v_new_watermark FROM RELIANT_DWH_SILVER.SILVER_STORES WHERE updated_at > v_watermark;
    IF v_new_watermark IS NULL THEN
        INSERT INTO RELIANT_DWH_BRONZE.SP_EXECUTION_LOG
            (sp_name, layer, target_table, started_at, ended_at, duration_secs, watermark_used, rows_merged, status, error_message)
        VALUES (v_sp_name, v_layer, v_target_table, v_started_at, NOW(),
            TIMESTAMPDIFF(SECOND, v_started_at, NOW()), v_watermark, 0, 'NOOP', NULL);
        SELECT CONCAT(v_sp_name, ': no new store updates since ', v_watermark) AS result;
        LEAVE proc_end;
    END IF;
    ...
    SELECT CONCAT(v_sp_name, ': ', v_rows, ' store rows upserted. Watermark=', v_new_watermark) AS result;
