USE RELIANT_DWH_GOLD;

DELIMITER $$

DROP PROCEDURE IF EXISTS RELIANT_DWH_GOLD.SP_LOAD_GOLD_FACT_INVENTORY$$

CREATE PROCEDURE RELIANT_DWH_GOLD.SP_LOAD_GOLD_FACT_INVENTORY()
BEGIN
    DECLARE v_started_at TIMESTAMP;
    DECLARE v_rows_merged INT DEFAULT 0;
    DECLARE v_sp_name VARCHAR(100) DEFAULT 'RELIANT_DWH_GOLD.SP_LOAD_GOLD_FACT_INVENTORY';
    DECLARE v_layer VARCHAR(20) DEFAULT 'GOLD';
    DECLARE v_target_table VARCHAR(100) DEFAULT 'RELIANT_DWH_GOLD.FACT_INVENTORY';
    DECLARE v_error_msg TEXT;
    
    DECLARE exit handler FOR SQLEXCEPTION
    BEGIN
        GET DIAGNOSTICS CONDITION 1 v_error_msg = MESSAGE_TEXT;
        INSERT INTO RELIANT_DWH_BRONZE.SP_EXECUTION_LOG
            (sp_name, layer, target_table, started_at, ended_at, duration_secs, watermark_used, rows_merged, status, error_message)
        VALUES (v_sp_name, v_layer, v_target_table, v_started_at, NOW(),
            TIMESTAMPDIFF(SECOND, v_started_at, NOW()), NULL, v_rows_merged, 'FAILED', v_error_msg);
    END;

    SET v_started_at = NOW();

    -- Insert/Update FACT_INVENTORY
    INSERT INTO RELIANT_DWH_GOLD.FACT_INVENTORY
        (date_key, product_key, store_key, quantity_sold, opening_stock, closing_stock)
    SELECT
        CAST(DATE_FORMAT(i.inventory_date, '%Y%m%d') AS UNSIGNED) AS date_key,
        p.product_key,
        s.store_key,
        i.quantity AS quantity_sold,
        i.opening_stock,
        i.closing_stock
    FROM RELIANT_DWH_SILVER.SILVER_INVENTORY i
    JOIN RELIANT_DWH_GOLD.DIM_PRODUCT p ON i.product_id = p.product_id
    JOIN RELIANT_DWH_GOLD.DIM_STORE s ON i.store_id = s.store_id
    ON DUPLICATE KEY UPDATE
        quantity_sold = VALUES(quantity_sold),
        closing_stock = VALUES(closing_stock),
        updated_at = NOW();

    SET v_rows_merged = ROW_COUNT();

    INSERT INTO RELIANT_DWH_BRONZE.SP_EXECUTION_LOG
        (sp_name, layer, target_table, started_at, ended_at, duration_secs, watermark_used, rows_merged, status, error_message)
    VALUES (v_sp_name, v_layer, v_target_table, v_started_at, NOW(),
        TIMESTAMPDIFF(SECOND, v_started_at, NOW()), NULL, v_rows_merged, 'SUCCESS', NULL);

    SELECT CONCAT(v_sp_name, ': ', v_rows_merged, ' rows merged') AS result;
END$$

DELIMITER ;
